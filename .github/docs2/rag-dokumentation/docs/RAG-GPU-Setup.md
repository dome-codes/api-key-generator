## RAG-Pipeline: GPU-Setup & Hardware-Entscheidungen
...
Dieses Dokument beschreibt die **Hardware- und GPU-Strategie** für unsere RAG-Pipeline. Ziel ist es, die begrenzten GPU-Ressourcen so zu partitionieren, dass:

- die **Dokumentenextraktion** (MinerU),
- die **Vektorerstellung & Suche** (Embedding & Reranking) und
- die **Antwortgenerierung** (LLM)

stabil und performant parallel betrieben werden können – ohne dass sich Workloads gegenseitig ausbremsen.
...
## 3. Architektur-Diagramm (Hardware-Sicht)

Das Diagramm zeigt den Datenfluss von der Roh-PDF bis zur fertigen Antwort aus Hardware-Perspektive:

- Welche Workloads auf GPU 0 vs. GPU 1 laufen
- Wie MinerU, Embedding, Reranker und LLM zusammenwirken
...
graph TB
    ...
    MinerU_Group -->|"Extrahertes Markdown"| Chunking
    Chunking --> EMB
...
## 4. Prozessbeschreibung (GPU-Perspektive)

### 4.1 Phase A: Ingestion (Datenaufnahme)

1. **MinerU (GPU 1, Slices 1–4):** PDFs werden parallel verarbeitet. MinerU extrahiert Layouts, Tabellen und Text in sauberes Markdown.
2. **Vektorisierung (GPU 1, Slice 5):** Der Text wird in Chunks zerlegt, vom Embedding-Modell in Vektoren übersetzt und in der Vektordatenbank gespeichert.

### 4.2 Phase B: Retrieval (Suche)

1. Die Nutzeranfrage wird ebenfalls vektorisiert (GPU 1, Slice 5).
2. Die Vektordatenbank liefert relevante Dokumentenabschnitte (Top-K Treffer).
3. Der **Reranker** (GPU 1, Slice 6) bewertet diese Abschnitte neu und filtert nur fachlich korrekte Informationen für das LLM.

### 4.3 Phase C: Generation (Antwort)
